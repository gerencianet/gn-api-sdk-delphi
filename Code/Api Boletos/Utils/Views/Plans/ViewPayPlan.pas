...
uses
  ...
  uCommonTypes, uPaymentClasses,
  XSuperJSON, XSuperObject;

type
  TViewPayPlan = class(TForm)
    GroupBox1: TGroupBox;
...
procedure TViewPayPlan.btnCancelRequestClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TViewPayPlan.btnConfirmRequestClick(Sender: TObject);
begin
...
procedure TViewPayPlan.ClearRequestfields;
begin
  txtName.Text :=  '';
  txtCPF.Text :=  '';
  txtEmail.Text :=  '';
...
procedure TViewPayPlan.FormCreate(Sender: TObject);
begin
  Self.Position := poScreenCenter;
  txtMessage.Text := '';
end;

function TViewPayPlan.GetBody: String;
var
...
function TViewPayPlan.GetIdent: String;
begin
