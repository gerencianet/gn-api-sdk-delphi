...
type
  TViewPayPlanCreditCard = class(TForm)
    GroupBox1: TGroupBox;
...
procedure TViewPayPlanCreditCard.btnCancelRequestClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TViewPayPlanCreditCard.btnConfirmRequestClick(Sender: TObject);
begin
...
procedure TViewPayPlanCreditCard.ClearRequestfields;
begin
  txtName.Text :=  '';
  txtCPF.Text :=  '';
  txtEmail.Text :=  '';
...
procedure TViewPayPlanCreditCard.FormCreate(Sender: TObject);
begin
  Self.Position := poScreenCenter;
end;

function TViewPayPlanCreditCard.GetBody: String;
var
  ClassChargeCreate                  : TPrivateCreditCardPayment;
  FAddress                           : TAddress;
  BodyRequest, BodyToken, BodyToken2,
  BodyPayment, BodyCreditCard        : ISuperObject;
...
begin
  ...
  BodyCreditCard  := BodyPayment.O['credit_card'];

  BodyCreditCard.Remove('installments');
  BodyPayment.SetData('credit_card', BodyCreditCard);

  Result := BodyRequest.AsJSON;
end;

function TViewPayPlanCreditCard.GetIdent: String;
begin
